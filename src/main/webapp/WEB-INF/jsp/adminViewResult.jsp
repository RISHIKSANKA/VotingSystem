...
<div class="jumbotron">
    <center>
        ...
        <h1 class="display-4"><a href="admin-view-result" class="btn btn-primary">RELOAD VOTING RESULT</a></h1>
    </center>
    <hr class="my-4">
    ...
    <div>
        <center>
            <h3 class="display-4">Vote cast details</h3>
        </center>
    </div>
    ...
    <div class="table-responsive">
        <table class="table table-striped table-bordered table-sm" id="dtBasicExample" cellspacing="0" width="100%">
            ...
            <tbody>
            <c:forEach items="${voteCastingDetailsList}" var="voteCastingDetails" varStatus="loop">
                <tr>
                    <td>${voteCastingDetails.candidate.loginId }</td>
                    <td>${voteCastingDetails.candidate.name}${' '}${voteCastingDetails.candidate.surname}</td>
                    <td>${voteCastingDetails.candidate.dLNum}</td>
                    <td>${voteCastingDetails.voteCastDate}</td>
                    <td>${voteCastingDetails.voter.loginId}</td>
                    <td>${voteCastingDetails.voter.name}${' '}${voteCastingDetails.voter.surname}</td>
                    <td>${voteCastingDetails.voter.dLNum}</td>
                </tr>
            </c:forEach>
            </tbody>
        </table>
